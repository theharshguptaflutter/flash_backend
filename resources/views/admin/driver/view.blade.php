...
<div class="content-wrapper">
    ...
    <section class="content-header">
        ...
        <div >
            <a class="btn btn-warning black_btn" href="{{ route('driver-awaiting-list') }}" title="Go back"> <i class="fa fa-angle-double-left"></i> Back</a>
        </div>
    </section>
                                   ...
                                   <!-- // driver document  start -->
                                    ...
                                    <div class="col-md-12 divi_list diriver_deatils_bottom">
                                     ...
                                     <?php if(isset($driver->user_id)){?>
                                      <form id="doc_status" method="post" action="{{route('driver-update',$driver->user_id)}}">
                                      @csrf
                                       <input type="hidden"   name="driver_detail_id" value="{{ isset($driver->id)?($driver->id):'' }}">
                                      <?php } ?>
                                       
                                      <div class="col-md-12 divi_list diriver_deatils_bottom">
                                      <div class="form-group">
                                        <label for="approval_status" class="control-label">Document status*</label>
                                        <select name="approval_status" class="form-control" id="approval_status" onchange="get_document_status(this.value)">
                                          <option value="">Select</option>
                                          <option <?php if(isset($driver->user->driver_approval) && $driver->user->driver_approval == "A"){ echo "selected"; }?> value="A">Approve</option>
                                          <option <?php if(isset($driver->user->driver_approval) && $driver->is_admin_approve == "R"){ echo "selected"; }?> value="R">Reject</option>
                                          <!-- <option <?php //if($driver->user->driver_approval == "P"){ echo "selected"; }?> value="P">Pending</option> -->
                                        </select>
                                      </div>
                                    </div> 

                                    <?php if(isset($driver->user->driver_approval) && $driver->is_admin_approve == "R") { ?>                                    
                                    <div class="col-md-12 divi_list diriver_deatils_bottom" id="reason_div">
                                      <div class="form-group">
                                        <label for="reject_document_reason" class="control-label">Reason*</label>
                                        <textarea type="text" class="form-control"  name="reject_document_reason" id="reject_document_reason" >{{isset($driver->reject_document_reason)?$driver->reject_document_reason:" " }}   </textarea >
                                      </div>
                                    </div>
                                    <?php } else{
                                     ?>
                                   <!-- -->
                                    <div class="col-md-12 divi_list diriver_deatils_bottom" id="reason_div" style="display:none;">
                                      <div class="form-group">
                                        <label for="reject_document_reason" class="control-label">Reason*</label>
                                        <textarea type="text" class="form-control"  name="reject_document_reason" id="reject_document_reason" >{{isset($driver->reject_document_reason)?$driver->reject_document_reason:" " }}   </textarea >
                                      </div>
                                    ...
                                    @if((isset($driver->is_admin_approve) && ($driver->is_admin_approve == 'R')) || (isset($driver->is_driver_complete) && ($driver->is_driver_complete == 'Y')) )
                                   <div class="card-footer clearfix">
                                        <button type="button" onclick="validateDocStatus()" name="update" id="update" class="btn btn-info update_btn">Update</button>                                    
                                    </div> 
                                    @endif  
